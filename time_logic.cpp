...
void TimeLogic::init(){
  if (RTC.isPresent()!=0) {
    RTC.getTime();
    LogHandler::logMsg(TIME_MODULE_NAME, F("Current hour: "), RTC.hour);
    LogHandler::logMsg(TIME_MODULE_NAME, F("Current minute: "), RTC.minute);
    LogHandler::logMsg(TIME_MODULE_NAME, F("Current second: "), RTC.second);
    RTC.startClock();
  } else {
